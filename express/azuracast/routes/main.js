...
router.get("/now_playing", async (req, res) => {
	...
	function getMediaUrl(val){
		return front_api_link + "/radio/now_playing/cover/" + val.split(`${azuracast_server}/api/station/tntr/art/`)[1];
	}

	result.now_playing.song.art = getMediaUrl(result.now_playing.song.art);
	result.playing_next.song.art = getMediaUrl(result.playing_next.song.art);

	for (let i of result.song_history){
		i.song.art = getMediaUrl(i.song.art);
	}
...
router.get("/now_playing/cover/:cover", async (req, res) => {
	try {
	const { cover } = req.params;
	await getMediaRequest(`${azuracast_server}/api/station/tntr/art/${cover}`,res,req);
