...
export default function ArtistPage({ params }) {
	...
	async function isArtistExist() {
		...
		const found = artists_list.find(
		(artist_obj) => artist_obj.title_min === decodeURIComponent(artist)
		);

		if (!found) {
			router.push("/sets");
		} else {
			setActArtistObj(found);
			setPageLoaded(true);
	...
	function formatDuration(seconds) {
		let durationSeconds = Math.floor(seconds);
		let durationMinutes = Math.floor(durationSeconds / 60);
		let durationHours = Math.floor(durationMinutes / 60);

		let totalMinutes = durationMinutes % 60;
		let totalSeconds = durationSeconds % 60;

		totalMinutes = totalMinutes <= 9 ? "0" + totalMinutes : totalMinutes;
		totalSeconds = totalSeconds <= 9 ? "0" + totalSeconds : totalSeconds;

		return durationHours > 0 ? `${durationHours}h${totalMinutes}` : totalSeconds > 0 ? `${durationMinutes}m${totalSeconds}s` : `${durationMinutes}m`;
	};

	function renderArtistLinks() {
		if (!act_artist_obj?.external_links) return null;

		return act_artist_obj.external_links.map((link, idx) => {
			if (!link) return null;
			let icon = Object.keys(brand_icons).find((key) =>
				link.includes(key)
			)
				? brand_icons[Object.keys(brand_icons).find((key) =>
					link.includes(key)
				)]
				: default_link_icon;
	...
	function renderArtistSets() {
		if (!artist_sets.length) return null;

		return artist_sets.map((set) => {
			const releasedAt = new Date(set.release_date).toLocaleDateString("fr-FR");
			const durationFormatted = formatDuration(set.duration);
			const isPlaying = media_played === set.media && !is_media_paused;
			...
			return (
				<article key={set.title_unique_name} className="episodes-comp">
					<hr/>
						...
						<div className="episodes-text">
							<div className="episode-title-header">
								<div className="episode-title-header-comp">
									...
									<Link href={`/sets/${set.artist_unique_name}/${set.title_unique_name}`}>	
									<h3 className={`episode-${set.title_unique_name} episode-title`}>{set.title}</h3>
									</Link>
									<p className="episode-duration">{durationFormatted}</p>
								</div>
								<p className="episode-published-at">Publié le {releasedAt}</p>
							</div>
							<hr/>
							<br/>
							<Link href={`/sets/${set.artist_unique_name}/${set.title_unique_name}`}>
								<p className={`episodes-desc episode-${set.title_unique_name}`}>{set.desc}</p>
							</Link>
						</div>
					</div>
